...
# 自作ライブラリ
from head_nod_analysis import setup_variable, feature_selection, process_data
from head_nod_analysis import get_feature
...
if __name__ == '__main__':
    ...
    data_set = 'main'
    SFM_threshold_list = [i / 1000 for i in range(0, 11)]

    # SFM閾値と選択される特徴量数(Excel出力用)
    output_SFM_feature_list = [SFM_threshold_list]
    select_feature_list = []
    ...
    for SFM_threshold in SFM_threshold_list:
        print(SFM_threshold)
        feature_file_name = 'WS' + str(N) + '_threshold' + str(threshold) + '_SFM' + str(SFM_threshold)

        # ウィンドウ処理
        answer_list, feature_list = [], []
        do_process_window()
        ...
        # 特徴量選択(SFM)
        analysis_data_file = path + '\\data_set\\analysis_files\\feature_selection\\' + sensor_name + '\\SFM\\' + feature_file_name
        rm_make_files(analysis_data_file)
        method = feature_selection.Embedded_Method(forest, X, y, analysis_data_file, SFM_threshold)
        X, feature_num = method.SFM()
        select_feature_list.append(feature_num)
    ...
    output_SFM_feature_list.append(select_feature_list)
    output_SFM_feature_pd = pd.DataFrame(data=output_SFM_feature_list, index=['SFM_threshold', 'select_feature_num'])  # SFM閾値と選択される特徴量数
    output_SFM_feature_pd.to_csv(path + '\\Execution_folder\\SFM比較\\SFM閾値ごとの特徴量数_ver2.csv')
