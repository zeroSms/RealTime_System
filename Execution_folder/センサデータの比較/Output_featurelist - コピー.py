...
from head_nod_analysis import get_feature_entropy
...
def do_process_window():
    ...
    for file_name in file_list:
        ...
        while len(data_queue) > N:
            ...
            if window:
                # 特徴量抽出
                feature_list.append(get_feature_entropy.get_feature(window, sensor_name))
...
if __name__ == '__main__':
    ...
    # 特徴量リスト
    get_feature_entropy.feature_name(sensor_name)
    # ウィンドウ処理
    do_process_window()
    ...
    # 正解データ取得
    X = pd.DataFrame(feature_list, columns=get_feature_entropy.feature_columns)
    ...
    if feature_check == '1':
        ...
        analysis_data_file = path + '\\data_set\\analysis_files\\feature_selection\\' + sensor_name + '\\センサデータの比較\\周波数領域'
        rm_make_files(analysis_data_file)
